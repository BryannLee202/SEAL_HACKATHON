...
:: 1. Kiem tra thu vien Frontend va BFF
...
if not exist "bff\node_modules" (
    echo [THONG BAO] Dang cai dat thu vien BFF...
    cd bff
    call npm install
    cd ..
)
...
echo [1/5] Dang xac thuc Ma tran Truy xuat Yeu cau (Traceability Matrix)...
...
echo [2/5] Dang chay toan bo 424 bai kiem thu Backend Spring Boot...
cd backend
call mvnw.cmd test
if %ERRORLEVEL% neq 0 (
...
echo [3/5] Dang chay toan bo 146 bai kiem thu Frontend UI va AI Engine...
cd frontend
call npm test -- --run
if %ERRORLEVEL% neq 0 (
...
:: 5. Kiem thu BFF (Jest) - tang nay truoc day bi bo sot hoan toan
echo [4/5] Dang chay toan bo 19 bai kiem thu BFF NestJS...
cd bff
call npm test
if %ERRORLEVEL% neq 0 (
    echo [THAT BAI] BFF Jest tests phat hien loi!
    cd ..
    pause
    exit /b 1
)
cd ..
echo [THANH CONG] BFF Tests: 19 tests PASS!
echo.

:: 6. Kiem tra kieu du lieu TypeScript & Build (Typecheck)
echo [5/5] Dang xac thuc kieu du lieu TypeScript va Bundle (npm run build)...
cd frontend
call npm run build
if %ERRORLEVEL% neq 0 (
...
echo   - BFF Jest Tests: 19 tests PASS (NestJS)
